...
import { getUserRoleFromToken, clearAuthToken } from "@/utils/auth";
...
export default function Sidebar() {
  ...
  // Identidade visual: Admin usa tom slate escuro, Vaqueiro mantem o verde institucional
  const sidebarBg = isAdmin ? "bg-admin-background" : "bg-green-background";

  /**
   * Menu do usuário logado.
   * Vaqueiro tem acesso restrito: Início, Dados diários, Meus Animais e Notificações.
   * Raças, Tipos de Animal, Funcionários e Relatórios são exclusivos do ADMIN.
   */
  const allMenuItems = [
    {
      name: "Início",
    ...
    {
      name: "Relatórios",
      link: "/dashboardUser/reports",
      icon: <BarChart3 size={ICON_SIZES.SM} />,
      adminOnly: true,
    },
  ];
