...
} from "@/hooks/queries/useAnimalSpecies";
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#2f9e41] resize-none"
            />
            {errors.description && (
...
export default function AnimalSpeciesPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<AnimalSpecies | null>(
    null,
  );
  ...
  const openEdit = (s: AnimalSpecies) => {
    setEditing(s);
    setModalOpen(true);
  };
  const showFeedback = (
