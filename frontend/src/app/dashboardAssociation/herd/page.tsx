      ...
      <header className="bg-white shadow-sm border-b border-slate-200 px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-4 bg-[#fdfbf7] px-4 py-2 rounded-lg border border-slate-200">
            <div className="text-right hidden md:block">
              <p className="text-xs text-slate-400 font-bold uppercase">Atualizado em</p>
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <section className="bg-white p-6 rounded-xl shadow-md border-t-4 border-[#d97706]">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 <div className="p-4 bg-slate-50 rounded-lg text-center hover:bg-slate-100 transition">
                     <p className="text-2xl font-black text-[#1e3a29]">{stats.lactatingCows.toLocaleString()}</p>
                     <p className="text-slate-500 text-sm font-medium">Vacas em Lactação</p>
                 </div>
                 <div className="p-4 bg-slate-50 rounded-lg text-center hover:bg-slate-100 transition">
                     <p className="text-2xl font-black text-slate-400">{stats.dryCows.toLocaleString()}</p>
                     <p className="text-slate-500 text-sm font-medium">Vacas Secas</p>
                 </div>
                 <div className="p-4 bg-slate-50 rounded-lg text-center hover:bg-slate-100 transition">
                     <div className="flex justify-center mb-1"><Baby className="w-5 h-5 text-amber-500"/></div>
                     <p className="text-2xl font-black text-[#d97706]">{stats.heifers.toLocaleString()}</p>
                     <p className="text-slate-500 text-sm font-medium">Novilhas</p>
                 </div>
                 <div className="p-4 bg-slate-50 rounded-lg text-center hover:bg-slate-100 transition">
                     <div className="flex justify-center mb-1"><Baby className="w-5 h-5 text-green-500"/></div>
                     <p className="text-2xl font-black text-[#4ade80]">{stats.calves.toLocaleString()}</p>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-100">
                <h3 className="text-lg font-bold text-[#1e3a29] mb-4">Volume de Leite Regional (7 dias)</h3>
