...
// @UseGuards(AuthGuard('jwt')) // Protege todas as rotas de coletas diárias
export class DailyCollectionsController {
  constructor(private readonly dailyCollectionsService: DailyCollectionsService) {}

 
  @ApiOperation({ summary: 'Responder formulário' })
  @ApiBearerAuth()
  @ApiResponse({ status: 201, description: 'Formulário respondido com sucesso' })
  ...
  async create(@Body() createDailyCollectionDto: CreateDailyCollectionDto) {
    try {
      const result = await this.dailyCollectionsService.create(createDailyCollectionDto);
      return {
  ...
  //////////////////////// Retorna todos os formulario cadastrados
  ...
  //////////////////// Checar se formulario do dia foi respondido
  ...
  async checkIfUserAlreadySubmitted(@Query('userId') userId: string) {
    try {
      ...
      const result = await this.dailyCollectionsService.checkIfUserAlreadySubmitted(userIdNumber);
  ...
  @ApiResponse({ status: 200, description: 'Formulário encontrado com sucesso' })
  ...
  @ApiOperation({ summary: 'Atualizar todos os dados de um formulário pelo ID' })
  @ApiBearerAuth()
  @ApiParam({ name: 'id', description: 'ID do formulário', type: Number })
  @ApiResponse({ status: 200, description: 'Formulário atualizado com sucesso' })
  @ApiResponse({ status: 400, description: 'Dados inválidos' })
  @ApiResponse({ status: 404, description: 'Formulário não encontrado' })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  @Put(':id')
  ...
  async update(
    ...
    try {
      const result = await this.dailyCollectionsService.update(id, updateDailyCollectionDto);
      return {
        statusCode: HttpStatus.OK,
        message: result.message,
        data: result.data,
  ...
  @ApiOperation({ summary: 'Buscar todos os formulários de um usuário pelo ID' })
  @ApiQuery({ name: 'userId', description: 'ID do usuário', type: Number })
  @ApiResponse({ status: 200, description: 'Formulários encontrados com sucesso' })
  @ApiResponse({ status: 404, description: 'Nenhum formulário encontrado para o usuário' })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  @Get('user/:userId')
